...
export function History() {
    ...
    const { data, isLoading, hasMore, mutate, loadMore } = useChatHistory({ pageSize: PAGE_SIZE })
    ...
    const handleExportHistory = async () => {
        ...
        setIsExporting(true)
        try {
            const chatsToExport = selectedChats.size > 0 ? Array.from(selectedChats) : displayChats.map(chat => chat.id)
            ...
            toast.loading(`Exporting ${chatsToExport.length} chats with messages...`, { id: toastId })
            ...
            toast.success(`Exported ${chatsWithMessages.length} chats with all messages!`, { id: toastId })
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">
                                {selectedChats.size > 0 ? `Export ${selectedChats.size}` : displayChats.length > 0 ? "Export All" : "Export"}
                            </span>
                        </Button>
                    </div>
                </div>
                            ...
                            {isLoading ? (
                                ...
                                displayChats.map((chat) => (
                                    <TableRow key={chat.id}>
                                        ...
                                        <TableCell className="font-medium truncate">{chat.title}</TableCell>
...
function DeleteHistoryDialog({ children, mutate }: { children: React.ReactNode, mutate: () => void }) {
