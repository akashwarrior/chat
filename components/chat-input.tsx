...
import { chatInputAtom } from "@/store/chat-input-store";
...
export function ChatInput({ isLoading, stop, submit }: ChatInputProps) {
    ...
    async (files: File[]) => {
      const validFiles = files.filter((file) => {
        if (!isSupportedAttachmentType(file.type)) {
          toast.error("Only PNG, JPG, JPEG, and PDF files are supported.");
          return false;
        }
  ...
  const buildMessagePayload = (text: string): Omit<UIMessage, "id"> => ({
    role: "user",
    parts: [
  ...
  const handleFormSubmit = async (event?: React.FormEvent<HTMLFormElement>) => {
    ...
    if (uploadQueue.length > 0) {
      toast.error("Please wait for uploads to finish before sending.");
      return;
    }
    ...
    const message = buildMessagePayload(trimmedInput);
